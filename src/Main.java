...
class Main {
	...
	void main(String[] args) {
		...
		do {
			System.out.print("1. Listado de datos de profesores (Realizado en le v1.0)\r\n" +
			                 ...
			                 "profesHorario.txt\r\n" +
			                 "4. Genera un archivo de texto con únicamente los horario de las clases que sean de " +
			                 "una\r\n" +
			                 "disciplina pedida al usuario por teclado, el archivo se llamará con el nombre de " +
			                 "la\r\n" + "disciplina.txt.\r\n" +
			                 "5. Realiza el punto 3 del ejercicio pero esta vez poniendo como carácter separador " +
			...
			switch (opc) {
				...
				case 4:
					...
					createHorariosAsignatura(docentes, asignatura);
					break;
				case 5:
					System.out.print("Introduce un horario(hh:mm a hh:mm): ");
					horario = sc.nextLine();
					createProfesHorario5(docentes, horario);
					break;
				default:
					break;
			}
	...
	public static
	void readFile(ArrayList<Docente> docentes, String path) {
		// TODO Auto-generated method stub
		String line = "";
		try {
	...
	public static
	void createProfe36(ArrayList<Docente> docentes) {
		...
		try {
			...
			for (int i = 0; i < docentes.size(); i++) {
				...
				if (horarios[0].contains("15:00 A 18:00") || horarios[1].contains("15:00 A 18:00")) {
					line = docentes.get(i).getId() + "|" + docentes.get(i).getNombre() + "|" + docentes.get(i)
					                                                                                   .getCurso() +
					       "|" + docentes.get(i).getHorario() + "|" + docentes.get(i).getJefe();
					bw.write(line);
					bw.newLine();
				}
			}
	...
	public static
	void createProfesHorario(ArrayList<Docente> docentes, String horario) {
		// TODO Auto-generated method stub
		...
		try {
			...
			for (int i = 0; i < docentes.size(); i++) {
				String horarios[] = docentes.get(i).getHorario().split(" y ");
				if (horarios[0].contains(horario) || horarios[1].contains(horario)) {
					line = docentes.get(i).getId() + "|" + docentes.get(i).getNombre() + "|" + docentes.get(i)
					                                                                                   .getCurso() +
					       "|" + docentes.get(i).getHorario() + "|" + docentes.get(i).getJefe();
					bw.write(line);
	...
	public static
	void createHorariosAsignatura(ArrayList<Docente> docentes, String asignatura) {
		// TODO Auto-generated method stub
		File file = new File("horariosAsignatura.txt");
		try {
			file.createNewFile();
		} catch (IOException e) {
			// TODO Auto-generated catch block
		...
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(file, false));
			for (int i = 0; i < docentes.size(); i++) {
				if (docentes.get(i).getCurso().equalsIgnoreCase(asignatura)) {
					line = docentes.get(i).getHorario();
					bw.write(line);
					bw.newLine();
				}
			}
	...
	public static
	void createProfesHorario5(ArrayList<Docente> docentes, String horario) {
		// TODO Auto-generated method stub
		File file = new File("profesHorario.txt");
		try {
			file.createNewFile();
